...
import {ConcealerService, DateService, SantanderFileService, ValidationService, YnabAccountsService, YnabTransactionsService} from '.';
import {SantanderFileParseConfig, TransactionDetail} from '../models';
import {SantanderInputFileResult} from '../models/santander-input-file-result.model';
...
export class SantanderInputService {
  constructor(
  ...
  async inputFile(fileContent: string, year: number, monthIdx: number, budgetId: string, accountId: string): Promise<SantanderInputFileResult> {
    ...
    const rawInput: TransactionInput[] = this.santanderFileService.asTransactions(fileContent, parseConfig)
